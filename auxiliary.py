...
def get_balance(account):
  try:
    out = subprocess.run(['cleos', '-u', EOS_RPC, 'get', 'currency', 'balance', 'volentixgsys', account],
                          timeout=TIMEOUT, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    out = out.stdout.decode('utf-8')
  except subprocess.TimeoutExpired as e:
...
def get_ledger_balance(key):
  try:
    data = requests.get('https://demux.vlabs.ninja:4000/ledger/balance/'+key)
  except Exception as e:
    print(e)
...
def compare_transactions(collection):
  execute_list = []
  with open('deductor/execute.list', 'r') as f:
    for line in f:
      execute_list.append(line.replace('\n', '').split('-')[0].strip())

  print("Execute_list: ", len(execute_list))

  executed_volentixgsys = []
  for document in collection.find({}):
    executed_volentixgsys.append(document['memo'].split('trx_id:')[1])

  print("Executed_volentixgsys: ", len(executed_volentixgsys))

  result = any(tr in execute_list for tr in executed_volentixgsys)
  if result:
      print("Yes, execute_list does contain all transactions in executed_volentixgsys")
  else:
      print("No, execute_list does not contain all transactions in executed_volentixgsys")

  # print(executed_volentixgsys, len(executed_volentixgsys))

def get_list_of_vtx(collection):
  accounts = set()
  for document in collection.find({"currency":"VTX"}):
    accounts.add(document['from'])
    accounts.add(document['to'])

  with open('eos_balances.list', 'w') as f:
    for acc in accounts:
      balance = get_balance(acc)
      print('%s - %s' %(acc, balance))
      f.write('%s - %s' % (acc, balance))

def get_list_of_ledgers(collection):
  keys = set()
  for document in collection.find():
    if document['trx_id'] in IGNORE or document['trx_id'] in NOT_SURE:
      continue
    else:
      keys.add(document['tokey'])
  ...
  for key in keys:
    balance = get_ledger_balance(key)
    if balance < 1:
      less_one[key] = balance
      print('%s : %s [%s]' % (key, balance, 'less_one'))
    elif balance > 0 and balance <= 1000:
      less_thousand[key] = balance
      print('%s : %s [%s]' % (key, balance, 'less_thousand'))
    elif balance > 1000:
      more_thousand[key] = balance
      print('%s : %s [%s]' % (key, balance, 'more_thousand'))
    else:
      other[key] = balance
      print('%s : %s [%s]' % (key, balance, 'other'))

  # if less_one:
  #   with open('less_one.list', 'w') as f:
  #     for key, balance in less_one.items():
  #       account = get_account(key)['account_names']
  #       if account:
  #         account = account[0]
  #       else:
  #         account = ''
  #       f.write('%s : %s [%s]\n' % (key, balance, account))
  # if less_thousand:
  #   with open('less_thousand.list', 'w') as f:
  #     for key, balance in less_thousand.items():
  #       account = get_account(key)['account_names']
  #       if account:
  #         account = account[0]
  #       else:
  ...
  #       f.write('%s : %s [%s]\n' % (key, balance, account))
  if more_thousand:
    print('More thousand list: ', len(more_thousand))
    with open('more_thousand.list', 'w') as f:
      for key, balance in more_thousand.items():
        time.sleep(0.5)
  ...
  if other:
    print('Other list')
    with open('other.list', 'w') as f:
      for key, balance in other.items():
        account = get_account(key)['account_names']
        if account:
          account = account[0]
        else:
...
def setup():
  ...
  # compare_transactions(collection)
  # get_list_of_vtx(collection)
  get_list_of_ledgers(collection)
